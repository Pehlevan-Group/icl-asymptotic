...
for i in range(len(dirs)):
    plt.plot(taus,np.mean(total_data[i,:,:], axis = 1),label = f'{dirs[i]}')
    plt.fill_between(taus,np.mean(total_data[i,:,:], axis = 1)-np.std(total_data[i,:,:], axis = 1),np.mean(total_data[i,:,:], axis = 1)+np.std(total_data[i,:,:], axis = 1),alpha=0.2)
...
plt.legend()
plt.savefig('equal_curve_again.png')