...
trainobject = FiniteSampler(n_points = N+1, n_dims= d, variable_context=True, eta_scale = sigma, w_scale = psi, batch_size = P, diversity = K, seed=None);
...
state, hist = train(config, data_iter=iter(trainobject), idg_iter = iter(idgobject), test_iter=iter(testobject), loss='mse', batch_size=int(0.1*P), test_every=500, train_iters=20000, optim=optax.adamw,lr=1e-4)
